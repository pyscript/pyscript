...
const listener = async event => {
  ...
  for (let {name, value, ownerElement: el} of $x(`./@*[${
    ...
    const runtime = await awaitRuntime(el.getAttribute(`${name}-env`) || name);
    const i = index++;
    try {
...
const mo = new MutationObserver(records => {
  for (const {type, target, attributeName, addedNodes} of records) {
      ...
      if (i) {
        ...
        for (const p of prefixes) {
          if (prefix === p) {
            ...
            if (type !== 'env') {
              const method = target.hasAttribute(attributeName) ? 'add' : 'remove';
    ...
    for (const node of addedNodes) {
      if (node.nodeType === 1) {
        if (node.matches(RUNTIME_SELECTOR))
          handle(node);
        else {
