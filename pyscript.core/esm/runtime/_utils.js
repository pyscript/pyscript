...
export const writeFile = (FS, path, buffer) => {
  ...
  return FS.writeFile([parentPath, name].join('/'), new Uint8Array(buffer), {canOwn: true});
};
...
const calculateFetchPaths = config_fetch => {
  ...
  for (const {files, to_file, from = ''} of config_fetch) {
    if (files !== undefined && to_file !== undefined)
      throw new Error(`Cannot use 'to_file' and 'files' parameters together!`);
    if (files === undefined && to_file === undefined && from.endsWith('/'))
      throw new Error(`Couldn't determine the filename from the path ${from}, please supply 'to_file' parameter.`);
  }
  /* c8 ignore stop */
  return config_fetch.flatMap(({from = '', to_folder = '.', to_file, files}) => {
      if (isArray(files))
...
const fetchResolved = (config_fetch, url) => fetch(
  absoluteURL(url, base.get(config_fetch))
);

export const base = new WeakMap;

export const fetchPaths = (module, runtime, config_fetch) => all(
  calculateFetchPaths(config_fetch).map(
    ({url, path}) => fetchResolved(config_fetch, url).then(getBuffer).then(
      buffer => module.writeFile(runtime, path, buffer)
    ))
);
