...
const queryTarget = (script, idOrSelector) => {
  ...
  return (
    root.getElementById(idOrSelector) ||
    $(idOrSelector, root)
  );
};
...
const execute = async (script, source, XWorker, isAsync) => {
  ...
  /* c8 ignore next */
  if (module.experimental) console.warn(`The ${script.type} runtime is experimental`);
  const [runtime, content] = await all([handled.get(script).runtime, source]);
  try {
    // temporarily override inherited document.currentScript in a non writable way
    // but it deletes it right after to preserve native behavior (as it's sync: no trouble)
    defineProperty(globalThis, 'XWorker', {configurable: true, get: () => XWorker});
    defineProperty(document, 'currentScript', {configurable: true, get: () => script});
...
export const handle = async script => {
  ...
  // just exactly like any other script would
    ...
    const {attributes: {async: isAsync, config, env, target, version}, src, type} = script;
    ...
    handled.set(defineProperty(script, 'target', targetDescriptor), details);

    if (targetValue)
      targets.set(script, queryTarget(script, targetValue));

    // start fetching external resources ASAP
    const source = src ? fetch(src).then(getText) : script.textContent;
    details.queue = details.queue.then(() => execute(script, source, details.XWorker, !!isAsync));
  }
};
