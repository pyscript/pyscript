...
export const runtime = new Proxy(new Map, {
  get(map, id) {
    if (!map.has(id)) {
      ...
      const url = /^https?:\/\//i.test(rest) ? rest[0] : runtime.module(...rest);
      map.set(id, {url, module: import(url), engine: runtime.engine.bind(runtime)});
    }
    const {url, module, engine} = map.get(id);
    return (config, baseURL) => module.then(module => {
