...
async def find_path_parallel(find_path_name):
    ...
    for name, graph_d in GRAPHS.items():
        ...
        for worker in our_workers:
            ...
            worker.set_cheats(to_js(cheats))
            print("set cheats")
...
@upytest.skip("Main thread only", skip_when=RUNNING_IN_WORKER)
async def test_find_path_parallel_cheating():
    """Send and receive graphs and paths

    Doesn't actually run Dijkstra's algorithm in the worker.
    It's run in the main thread instead,
    and the results are given to the worker to return.

    """
    await find_path_parallel("cheating_dijkstra_path")


@upytest.skip("Main thread only", skip_when=RUNNING_IN_WORKER)
async def test_find_path_parallel():
    """
    Send and receive graphs, and find paths in them

    The graphs were generated by ``networkx``.

    Pathfinding is done in parallel. That's the way you
