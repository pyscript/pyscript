...
test("MicroPython display", async ({ page }) => {
  ...
  await expect(body.trim()).toBe(
    ["M-PyScript Main 1", "M-PyScript Main 2", "M-PyScript Worker"].join("\n"),
  );
});
...
test("Pyodide + terminal on Main", async ({ page }) => {
  await page.goto(
    "http://localhost:8080/tests/javascript/py-terminal-main.html",
  );
...
test("Pyodide + terminal on Worker", async ({ page }) => {
  await page.goto(
    "http://localhost:8080/tests/javascript/py-terminal-worker.html",
  );
  await page.waitForSelector("html.ok");
});
...
test("Pyodide + terminal process with leading spaces", async ({ page }) => {
  await page.goto(
    "http://localhost:8080/tests/javascript/py-terminal-process.html",
  );
  await page.waitForSelector("html.ok");
});
...
test("MicroPython using named Pyodide Worker", async ({ page }) => {
  await page.goto(
    "http://localhost:8080/tests/javascript/workers/mpy/index.html",
  );
...
test("MicroPython creating a named Pyodide Worker", async ({ page }) => {
  await page.goto(
    "http://localhost:8080/tests/javascript/workers/create_named/index.html",
  );
  await page.waitForSelector("html.pyodide_version");
});
...
test("Pyodide using named MicroPython Worker", async ({ page }) => {
  await page.goto(
    "http://localhost:8080/tests/javascript/workers/py/index.html",
  );
...
test("MicroPython Editor setup error", async ({ page }) => {
  await page.goto(
    "http://localhost:8080/tests/javascript/issue-2093/index.html",
  );
...
test("Pyodide media module", async ({ page }) => {
  ...
  const isSuccess = await page.evaluate(() =>
    document.documentElement.classList.contains("media-ok"),
  );
