...
def sign_clicked(args):
    ...
    if args.target.id == "equal-to":
        ...
        if current[0] == "-":
            ...
            document.getElementById("typing-text").innerText = "-" + str(
                what_type(current)
            )
...
def change_theme(all_btns, all_btns_num, theme="light"):
    document.getElementById("light").style.color = (
        "#060709" if theme == "light" else "#474B52"
    )
    document.getElementById("dark").style.color = (
        "#474B52" if theme == "light" else "white"
    )
    document.getElementById("main").style.backgroundColor = (
        "white" if theme == "light" else "#060709"
    )
    document.getElementById("theme-buttons").style.backgroundColor = (
        "#F9F9F9" if theme == "light" else "#292D36"
    )
    document.getElementById("main").style.color = (
        "#060709" if theme == "light" else "white"
    )
    document.getElementById("btn-wrapper").style.backgroundColor = (
        "#F9F9F9" if theme == "light" else "#292D36"
    )
    document.getElementById("bottom-line").style.backgroundColor = (
