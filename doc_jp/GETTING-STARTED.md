...
`<py-repl>` タグは、コードエディタとしてページに表示されるREPLコンポーネントを作成し、インラインで実行可能なコードを記述できるようにします。

## py-config タグ
PyScriptアプリケーションに関する一般的なメタデータをYAML形式で設定・構成するには、`<py-config>`タグを使用します。YAML に慣れていない場合は、Red Hat の [YAML for beginners](https://www.redhat.com/sysadmin/yaml-beginners)ガイドを読んで、より詳しい情報を得ることを検討してください。

`<py-config>` タグは以下のように使用することができます。
...
`<py-config>`は以下のオプションをサポートしています :

  * `autoclose_loader` (boolean): `false`に設定したとき、PyScript は起動操作が終了してもローディングスプラッシュ画面を閉じません。
  * `name` (string): アプリケーションの名前。このフィールドには任意の文字列を指定することができ、アプリケーション作成者が独自のカスタマイズを行うために使用します。
  * `version` (string): アプリケーションのバージョン。 このフィールドには任意の文字列を指定きます。アプリケーションの作者が独自のカスタマイズのために使用するものであり、PyScriptのバージョンとは関係ありません。
  * `runtimes` (List of Runtimes): ランタイムの設定リスト。各Runtimeは以下のフィールドを想定しています。:
    * `src` (string, Required): ランタイムのソースを示すURL。
    * `name` (string): ランタイムの名前。このフィールドには任意の文字列を指定することができ、アプリケーション作成者が独自のカスタマイズを行うために使用します。
    * `name` (string): ランタイムがサポートするプログラミング言語。このフィールドはアプリケーションの作者が明示的に使用するものです。現在のところ、PyScript の動作に影響を与えるものではありません。


## ビジュアルコンポーネントタグ

以下のタグを使用すると、HTMLページに視覚的な属性を追加できます。

| Tag             | Description |
| ---             | ----------- |
| `<py-inputbox>` | ユーザーに入力を促すために使用するインプットボックスを追加します。 |
| `<py-box>`      | 1つまたは複数のビジュアルコンポーネントをホストするために使用できるコンテナオブジェクトを作成します。|
| `<py-button>`   |ボタンにラベルを追加し、`on_focus` や `on_click` などのボタンに対するアクションのイベントハンドラを追加します。 |
| `<py-title>`    | タグ内のテキストをページタイトルとしてスタイル付けする静的テキストのタイトルコンポーネントを追加します。 |

```{note}
上記のすべての要素は実験的なものであり、完全な機能で実装されているわけではありません。ビジュアルコンポーネントがより成熟するまでは、APIやフルサポートが変更されたり削除されたりする可能性があることを理解した上で、これらを使用してください。
```
